...
const CheckoutTable = ({ cart, setGetProductId }) => {
  ...
  const product = products.find(
    (product) => product?._id === cart?.mainProductId
  );
  ...
  return (
    <div className="flex justify-between items-center pt-4 border-b pb-1 border-gray-300">
      ...
      <p className="flex items-center gap-x-2">
        <span className="inline-block">{product?.title.slice(0, 47)}..</span>
        <sub className="text-md text-blue-500">(x{cart.selectedQuantity})</sub>
      </p>
      <p>
